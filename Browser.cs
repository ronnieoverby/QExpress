...
order by owner, object", timeout);
		...
		public StringCollection GetActionList (TreeNode node)
		{
			...
			if (sn.type == "U" || sn.type == "S" || sn.type == "V")
			{
			    ...
			    output.Add(string.Format("sp_help '{0}'", sn.safeName));
				if (sn.type != "V")
				{
				    output.Add(string.Format("sp_helpindex '{0}'", sn.safeName));
					output.Add (string.Format("sp_helpconstraint '{0}'", sn.safeName));
					output.Add (string.Format("sp_helptrigger '{0}'", sn.safeName));
				}
